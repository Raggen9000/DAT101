...
export const GameProps = {
  ...
  score: 0,
  bestScore: 0,
  sounds: {countDown: null, food: null, gameOver: null, dead: null, running: null, flapLyd: null}
};
...
function loadGame() {
  ...
  GameProps.sounds.flapLyd = new libSound.TSoundFile("./Media/flapLyd.mp3");
...
function onKeyDown(aEvent) {
  switch (aEvent.code) {
    case "Space":
      if (!GameProps.hero.isDead) {
        GameProps.hero.flap();
        GameProps.sounds.flapLyd.stop();
        GameProps.sounds.flapLyd.play();
      }
      break;
  }
}

//fungere nå, men første gang spillet loader er lyden forsinket. trur det er på grunn av 

function countDownVerson2(){
  const countSound = GameProps.sounds.countDown;
...
document.addEventListener("click", countDownVerson2)
